...
{
  "decision": "block",
  "reason": "AUTO-SAVE checkpoint. 1) Write AAAK diary entry via mempalace_diary_write(agent_name='solar'). 2) Save key topics, decisions, code to MemPalace drawers. 3) Update KG if new entities/relationships emerged (mempalace_kg_add). Continue after saving."
}
HOOKJSON
else
