...
- **History audit basis:** full commit history swept — all 493 baseline commits (Sihao) and all 401 migration commits (Suraj) were reviewed and reconciled against the feature table below; attribution is per-commit evidence, not tree inspection alone
...
| Ingestion | AI Influence Report Center UI | Collected X/YouTube digests + transcripts | HTML report-center: video library, channel tabs, month filtering over ingested content | Baseline (Sihao) |
...
| Research & Synthesis | Survey-writing pipeline | Evidence graph + writer backends (pane/human packet backends) | Long-form survey line (largest single named baseline workstream, ~40 commits): planner, section compiler, golden-style gate, chief-editor gate, revision loops, auto-repair | Baseline (Sihao) |
...
| Research & Synthesis | understand-anything runtime | Arbitrary corpora via capability capsules (chat, domain-analysis, indexer, knowledge-graph, tour builders) | Capsule-driven comprehension/indexing runtime | Baseline (Sihao) |
| Research & Synthesis | html-anything renderer | Structured content → HTML profile templates | Adapter + profiles rendering arbitrary artifacts to HTML (`html_anything_adapter.py`) | Baseline (Sihao) |
...
| Model Providers | Codex CLI (gpt-5.5-codex; gpt-5.3-codex-spark builder pool) | Local `codex` CLI session; read-only sandbox execs | Premium coding/architecture reasoner via chain-watcher bridge; tiered token budgets (S=4000/A=2000/B=none); fail-closed route selectors; Spark builder pool + backlog-aware operator autoscaling | Baseline (Sihao) — hardened by Suraj |
| Model Providers | Codex web-search & reasoning-effort controls | `codex --search` + effort level from config/settings | Runtime defaults, settings store round-trip, and dashboard Settings UI for search/effort | Added by Suraj |
...
| Model Providers | ⚠ Partial / not integrated | — | **Kimi/Moonshot, MiniMax, cloud Qwen (DashScope): NOT integrated** (Kimi/Qwen appear only as X-ingestion handles; the Feb-era Brain-Router MiniMax adapter was deleted upstream). **OpenRouter:** no operator-registry entry, but shipped code probes `OPENROUTER_API_KEY` (`webwright_adapter.py`, vendored autoresearch). Vendored autoresearch also carries **OpenCode** and **claude-mimo (Xiaomi MiMo)** agent CLIs + MiniMax tool-call error handling — lab-grade, unregistered | — |
...
| Core Engine (Solar-Harness) | Pane orchestration & leases | Persona + workdir + pane id + dispatch_id | tmux pane launch; single-owner pane leases, TTL 600s, release requires matching dispatch_id | Baseline (Sihao) — Codex pane runtime selector (`SOLAR_PANE_RUNTIME`) added by Suraj |
...
| Core Engine (Solar-Harness) | Capsules | 8-field capsule schema (goal/facts/changes/risks/questions/next/round/topology) | Structured handoff artifacts between recursion rounds; skill→capsule compiler; execution gate blocks on missing/invalid capsule | Baseline (Sihao) — hardened by Suraj (read-only audit capsule, capsule proof sidecars) |
...
| Core Engine (Solar-Harness) | TS actor-host runtime + core/ subsystems | Agent message protocol; 5-phase flow DAG (P1–P5, gates G1/G2) | `core/` orchestrator: buildGraph/executeGraph, pause/resume/reroute, debate + vote modes; TS coordinator **dual-write log-only** unless flag-enabled. ~40 further TS subsystems ship under `core/` (TVS terminal visual renderer — was a baseline release gate —, hive, evolver, ontology, memory, smi, llm, mcp) | Baseline (Sihao) |
| Core Engine (Solar-Harness) | Symphony scheduler | Schedules, issue adapters, workspace hooks | Higher-level workspace orchestration: scheduler, runner, hooks, issue adapter, D6 guard, no-live-pane-mutation invariant (ADR: `symphony-integration-adr.md`) | Baseline (Sihao) |
| Core Engine (Solar-Harness) | Telemetry & data plane | `telemetry.sh`, `resource_telemetry.py`, run DB | Runtime/resource telemetry with concurrency-safe data-plane DB | Baseline (Sihao) |
| Core Engine (Solar-Harness) | Autoresearch (vendored) | Issue-loop specs; agent CLIs: claude, claude-mimo, codex, deepseek, opencode; `OPENROUTER_API_KEY` | Vendored autonomous issue-loop runner + pane-execution optimizer (`harness/vendor/autoresearch/`) | Baseline (Sihao) |
| Core Engine (Solar-Harness) | Benchmark subsystem | Terminal-Bench / Harbor adapters | `harness/lib/benchmark/` (terminal_bench, harbor_adapter, solar_solver); was wired as a baseline release-gating step | Baseline (Sihao) |
| Core Engine (Solar-Harness) | Secret-scan proof gate | Node-verdict dispatch outputs | Deterministic secret-scan guard in `graph_node_dispatcher`; emits `guard_decision`/`resource_binding` sidecars, blocks verdict on secret match | Added by Suraj |
| Core Engine (Solar-Harness) | Ops tooling | — | Nightwatch daemon, intent gateway / rawintent front door, sprint-mirror audit tool | Baseline (Sihao) |
| Dashboard / GUI | Status-server API | HTTP + SSE on loopback; `X-Solar-Token` / `?token=` | Core server + `/healthz`/`/events` are baseline; the sprints, per-sprint projection snapshot/delta (SSE diff emitter), intake, gate-verdict, deliverables, settings and usage endpoints (+1.8k lines) plus the loopback auth token are migration work | Core server Baseline (Sihao); API surface & security token Added by Suraj |
| Dashboard / GUI | Auth / sign-in subsystem | Device-code login; host-credential reuse | `/auth/status`, `/auth/login`, `/auth/login/status`, `/auth/reuse-host-creds` + AuthGate UI (subscription-first, degrade-open) | Added by Suraj |
| Dashboard / GUI | Operational surfaces | Settings write path (per-agent models, lab matrix, API keys), activity heatmap, per-sprint/per-model token usage, reversible session archive | Pre-overhaul dashboard surfaces retained in the current GUI (incl. 13s→22ms `/sprints` optimization) | Added by Suraj |
...
| Packaging & Distribution | Kernel overlay generator | `kernel/kernel.manifest` fragments + allowlists | Generates `~/.claude/solar/SOLAR.md` per selected components; single sentinel import block in `~/.claude/CLAUDE.md`, byte-preserving edits | Kernel *content* Baseline (Sihao); generator, manifest, allowlists & sentinel-block machinery built by Suraj (no `kernel/` exists on baseline) |
| Packaging & Distribution | `solar` CLI lifecycle surface | `solar status / backup / restore / ui / harness <verb>` | Terminal status view, backup/restore of user data, UI-lite terminal dashboard (`solar_ui_lite.py`), harness verb passthrough | Added by Suraj |
| Packaging & Distribution | DB schema versioning & migrations | `lib/installer/migrate.sh`; global `schema_version` | Migrations runner so upgrades evolve the DB safely ("defuse the upgrade time-bomb") | Added by Suraj |
...
| Memory & Knowledge | Experience memory library + livework | JSONL stores; intake state machine | Cross-sprint "Solar Experience Memory Library" (`harness/lib/experience/`) + livework intake state machine & dispatch visibility | Baseline (Sihao) |
| Memory & Knowledge | Obsidian wiki integration | Obsidian vault | Wiki bridge/export/upload-backfill + wiki quality gate — distinct from MemPalace | Baseline (Sihao) |
...
| FT-29 | Packaging | Install matrix full | `bash scripts/smoke-install-matrix.sh full-non-rust` (after D-001 fix, re-run `minimal` too). Include one leg on a PEP-668-managed Python (fresh Ubuntu 24.04) — that environment was the rc.7 trigger | | |
...
| FT-39 | Dashboard | Auth / sign-in subsystem | Start status-server; exercise `/auth/status` → device-code `/auth/login` + `/auth/login/status` → `/auth/reuse-host-creds`; kill server mid-session and verify AuthGate degrade-open | | |
| FT-40 | Model Providers | Codex web-search & reasoning-effort | Set search/effort in dashboard Settings → verify settings persistence → dispatch a codex node → confirm `--search`/effort flags appear in the dispatch ledger | | |
| FT-41 | Packaging | `solar` CLI: status / backup / restore / UI-lite | `solar status`; `solar backup` → mutate config/db → `solar restore` → verify round-trip; `bash scripts/check-solar-ui-lite.sh` | | |
| FT-42 | Packaging | DB schema migrations on upgrade | Install an older receipt/db state → `solar update` → verify `schema_version` bump + data preserved (`lib/installer/migrate.sh`) | | |
| FT-43 | Core Engine | Secret-scan proof gate | Craft a node output containing a fake credential → dispatch → verify `guard_decision` sidecar blocks the verdict; clean output passes | | |
| FT-44 | Core Engine | Coordinator self-completion ("Option A") | `SOLAR_COORD_MULTITASK_SELFCOMPLETE=1` (default OFF); approve a sprint → verify pool self-completes it, pgrep duplicate-guard holds, pool self-exits on terminal DAG | | |
| FT-45 | Core Engine | Eval-pool dispatch + Sonnet evaluator | `SOLAR_GRAPH_EVAL_OPERATOR_POOL` on; run dispatch-evals through the operator pool; verify bounded eval-dispatch escalation + per-node attribution in the ledger | | |
| FT-46 | Core Engine | Structured-output validation + repair-eval fencing | Feed a malformed build/closeout node output → validation rejects; force repeated repair evals → verify per-generation fencing + blocked-DAG closure | | |
| FT-47 | Core Engine | Benchmark subsystem | Run a `harness/lib/benchmark/` smoke (terminal_bench/harbor adapter dry path, `solar_solver`) | | |
| FT-48 | Core Engine | Autoresearch vendored runner (lab) | `harness/vendor/autoresearch/run.sh` with a configured agent CLI; if `OPENROUTER_API_KEY` set, verify that path; expect lab-grade, unregistered | | |
| FT-49 | Memory & Knowledge | Obsidian wiki integration | `cd harness && bash test-obsidian-wiki-integration.sh`; live: bridge/export/backfill against a scratch vault | | |
| FT-50 | Research & Synthesis | Survey pipeline gates | `python3 -m pytest harness/tests -k "survey or golden or gate_verdict" -x`; live: one survey run through golden-style + chief-editor gates | | |
| FT-51 | Dashboard | Operational surfaces | Settings round-trip (per-agent models, lab matrix, API keys); activity heatmap renders; token usage matches ledger; session archive is reversible | | |
| FT-52 | Ingestion | Report Center UI | Open the report-center HTML over ingested data; verify video library, channel tabs, month filter | | |
...
### 5.5 Out of scope — removed, superseded, or dead-end surfaces (do NOT test)

From the full-history audit; these appear in commit history but must not enter the test matrix.

**Removed/superseded within the baseline's own history (Sihao):**
- Solar v2.x framework surface (slash commands `/status`/`/stats`/`/banner`/`/checkpoint`, `@Agent` syntax, work-mode switching, Flow Engine, Skill Market) — replaced wholesale by the harness
- Brain Router MCP + multi-provider adapter framework incl. the MiniMax adapter (`MINIMAX_API_KEY`) — deleted upstream; only `harness/brain/lessons.jsonl` remains
- Moltbot office integration; skin-check local-model skill; the early "极简监控面板" web dashboard

**Removed by the migration branch (Suraj) — superseded, not testable:**
- `deploy/` install-deps/sync-config infra → superseded by the installer; `.github/workflows/solar-nightly-release.yml` → superseded by the new CI gates
- Mac-mini sync scripts (`Macmini-2-Macbook.sh`, sync-auditor skill), `harness/release/artifacts/` tarballs, demo dashboards, ~1,990 runtime/sprint-artifact paths (the WS0 purge — state, not features)

**Dead ends inside the 401 migration commits:**
- Stellven-org bootstrap pins — superseded by the repoint to `suraj-subrahmanyan/OpenSolar`; residue already tracked as D-002; do not exercise the Stellven fetch path
- Agent-status roster GUI — replaced by the DAG plan flow
- `SOLAR_GRAPH_BUILDER_OPERATOR_POOL` graph-dispatch builder pool — built, defaulted off, then deliberately bypassed by Option A; test the multi-task-pool route (FT-44) instead
- Fabricated-metrics demo dashboard page (removed); Opus evaluators (retired in favor of Sonnet, FT-45)
