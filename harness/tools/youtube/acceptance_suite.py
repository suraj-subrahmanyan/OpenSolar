...
from youtube.pollution_repair import repair_pollution, verify_repair
...
def run_pollution_report(root: Path, report_dir: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as td:
        ...
        conn.commit()
        before = conn.execute("SELECT COUNT(*) FROM youtube_transcripts WHERE transcript_status='missing' AND clean_path IS NOT NULL").fetchone()[0]
    ...
    report = {
        ...
        "report": "pollution",
        "generated_at": _now(),
        "ok": before == 165 and repair.total_repaired == 165 and remaining == 0,
        "seed_count": before,
