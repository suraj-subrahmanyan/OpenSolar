...
def enrich_graph(graph: dict[str, Any], source_text: str = "",
    ...
    Planner-declared capabilities are authoritative. Earlier versions unioned
    inferred capabilities into explicit node requirements, which could let broad
    contract text such as browser/research/document rules poison otherwise
    simple implementation nodes and make them unschedulable. Only infer when the
    field is missing, unless overwrite=True is explicitly requested.
    """
    ...
    for node in nodes:
        ...
        elif had_required_capabilities:
