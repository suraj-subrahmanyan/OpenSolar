...
if [[ -x "$HOME/.solar/harness/update-kpi.sh" ]] || true; then
  KPI_FILE="$KPI_FILE" SPRINTS_DIR="$SPRINTS_DIR" SID="$SID" python3 << 'PYEOF' 2>/dev/null || true
import json, os, glob, datetime

kpi_file = os.path.expanduser(os.environ["KPI_FILE"])
sprints_dir = os.path.expanduser(os.environ["SPRINTS_DIR"])
sid = os.environ["SID"]
