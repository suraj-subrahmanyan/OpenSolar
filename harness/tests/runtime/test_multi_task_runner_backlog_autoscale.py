...
def test_schedule_once_skips_node_with_existing_active_task(monkeypatch, tmp_path):
    graph_path = tmp_path / "sprint-x.task_graph.json"
    ...
    monkeypatch.setattr(mtr, "graph_files", lambda explicit: [graph_path])
    monkeypatch.setattr(mtr, "load_graph", lambda path: graph)
    monkeypatch.setattr(mtr, "ready_nodes", lambda payload: [{"id": "N1"}])
    monkeypatch.setattr(mtr, "recover_quota_failed_nodes", lambda path, payload: 0)
    monkeypatch.setattr(mtr, "launch_guard", lambda *a, **k: {"ok": True, "reason": "ready"})
    monkeypatch.setattr(
        mtr,
    ...
    monkeypatch.setattr(mtr, "status_summary_for_graph", lambda path: {"sid": "sprint-x", "ready": ["N1"]})
    monkeypatch.setattr(mtr, "list_harness_panes", lambda: [])
    monkeypatch.setattr(mtr, "recent_dispatch_rows", lambda: [])
    monkeypatch.setattr(mtr, "capability_summary", lambda rows=None: {"ok": 1, "warn": 0, "error": 0})
    monkeypatch.setattr(mtr, "cached_status_summaries_for_graphs", lambda paths: [])
    monkeypatch.setattr(mtr, "effective_scheduler_max_workers", lambda default: default)
    monkeypatch.setattr(mtr, "launch_node", lambda *a, **k: launches.append("N1") or {"id": "task-N1"})
    ...
    assert result["skipped"] == [
        {
            ...
            "errors": ["PLAN_CERTIFICATE_MISSING:?"],
        }
    ]


def test_schedule_once_allows_fixed_contract_graph_when_validator_enabled(monkeypatch, tmp_path):
    graph_path = tmp_path / "sprint-fixed.task_graph.json"
    graph = {
        "sprint_id": "sprint-fixed",
    ...
    monkeypatch.setattr(mtr, "status_summary_for_graph", lambda path: {"sid": "sprint-fixed", "ready": ["N1"]})
    monkeypatch.setattr(mtr, "list_harness_panes", lambda: [])
    monkeypatch.setattr(mtr, "recent_dispatch_rows", lambda: [])
    monkeypatch.setattr(mtr, "capability_summary", lambda rows=None: {"ok": 1, "warn": 0, "error": 0})
    monkeypatch.setattr(mtr, "cached_status_summaries_for_graphs", lambda paths: [])
    monkeypatch.setattr(mtr, "effective_scheduler_max_workers", lambda default: default)
    monkeypatch.setattr(mtr, "launch_node", lambda *a, **k: launches.append("N1") or {"id": "task-N1", "node_id": "N1"})

    result = mtr.schedule_once(args)

    assert launches == ["N1"]
    assert result["launched"] == [{"id": "task-N1", "node_id": "N1"}]
