#!/usr/bin/env python3
"""F-CLASS-21 gate_replay driver — a write outside the declared roots blocks the gate.

The manifest records an observed write outside every declared artifact root
(ARTIFACT_ROOT_VIOLATION) and _evaluate_proof_obligations blocks the node even
though its declared obligation is satisfied. Fault (SOLAR_GATE_LEDGER=0): the
consult is off, the contamination is invisible, and the gate passes.
"""
import json
import os
