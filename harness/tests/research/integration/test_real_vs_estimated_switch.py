...
def test_l1_provider_usage_ledger(tmp_path):
    ...
    assert metrics["total_tokens"] == metrics["total_token_consumption"]
    assert metrics["ledger_path"].endswith("model_usage.jsonl")
    assert metrics["ledger_lines"] == 1
...
def test_l3_estimated_no_provider(tmp_path):
    ...
    assert "sprint_id" in metrics
    assert "generated_at" in metrics
    assert metrics["total_tokens"] == metrics["total_token_consumption"]
    assert metrics["ledger_path"] == ""
    assert metrics["ledger_lines"] == 0
