...
CREATE TABLE IF NOT EXISTS bl_message_tasks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    task_id TEXT UNIQUE,      -- message-listener task id
    message_id TEXT,          -- iMessage or external message id
    source TEXT CHECK(source IS NULL OR source IN ('imessage', 'gmail', 'telegram', 'webhook', 'manual')),
    source_id TEXT,           -- 原始消息 ID，用于去重
    sender TEXT,
    content TEXT NOT NULL,
    parsed_intent TEXT,  -- 解析出的意图
    intent_type TEXT,         -- task/query/control
    intent_action TEXT,       -- weather_query, list_backlog, etc.
    intent_params TEXT,       -- JSON payload
    priority DEFAULT 50,      -- numeric backlog priority or listener label
    status TEXT DEFAULT 'pending' CHECK(status IN ('pending', 'queued', 'processing', 'completed', 'failed', 'cancelled', 'running', 'done', 'deferred')),
...
CREATE TABLE IF NOT EXISTS bl_message_triggers (
    ...
    action TEXT,  -- 执行的动作: 'skill', 'shortcut', 'agent', 'script'
    action_target TEXT,  -- 具体目标: '/weather', 'solar_get_weather', '@Coder'
    contact_name TEXT,
    contact_phone TEXT,
    contact_email TEXT,
